...
### Version 1.1.0-dev3 — 2026-05-26
**Development build for testing.**

#### New Features
- **Customizable Hotkeys** — All BOA hotkeys (e.g., Bulk Sheet Organizer, Move Sheet Left/Right) are now fully exposed in NVDA's Input Gestures dialog under the "Better Office Accessibility" category, allowing users to safely rebind them to their preference.

#### Bug Fixes
- **Excel: Selection Double-Announcement** — Fixed an issue where BOA would redundantly announce multi-cell selections that NVDA natively handles (e.g., via Shift+Arrow keys). BOA now detects manual key holds and only announces when a selection jump occurs programmatically (e.g., via the Go To box or Name Box).
