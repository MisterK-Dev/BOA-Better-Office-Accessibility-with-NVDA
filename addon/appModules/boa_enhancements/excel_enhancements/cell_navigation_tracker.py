...
_isDriftTimerRunning = False
_pending_injection_address = None

def _drift_poll_loop():
	global _isDriftTimerRunning
	if not _isDriftTimerRunning:
		return
	try:
					...
					if still_mismatched:
						...
						try:
							...
							if not isNativeSuccess:
								...
								if val is None:
									val = ""
								elif type(val) is float and val.is_integer():
									val = int(val)
							...
							try:
								if _cached_excel_app.Selection.Cells.Count > 1:
									...
									if sel_addr != _last_announced_address:
										...
										import ui
										# Translators: Announced when a cell or range is selected.
...
def suspend_tracker_and_release():
	...
	_isDriftTimerRunning to False when Excel loses focus, we preserve CPU. The tracker will 
			...
			if getattr(sel, 'Cells', None):
				...
				if count == 1 and _last_selection_count > 1:
					if boa_config.get_feature_state("excel", "unselect_tracking"):
						import ui
						# Translators: Announced when a selection is cleared and nothing is selected.
...
class CellNavigationTracker(object):
					...
					if sel.Cells.Count > 1:
						...
						if isMerged:
							if boa_config.get_feature_state("excel", "merged_cell_tracking"):
								...
								# Translators: Announced when navigating into a merged cell.
	...
	def check_hidden_skip(excel):
		...
		try:
			...
			if _last_focused_row is not None and _last_focused_col is not None:
				...
				if boa_config.get_feature_state("excel", "hidden_row_skip"):
					# ---------------- ROW JUMPS ----------------
					if not col_changed and row_changed:
						...
						if max_r - min_r > 1:
							...
							if hidden_count > 0:
								if not fragmented:
									if hidden_count == 1:
										# Translators: Announced when navigating past a single hidden row.
										...
										if first_hidden == min_r + 1 and last_hidden == max_r - 1 and (max_r - min_r - 1) >= 500:
											# Translators: Announced when navigating past multiple hidden rows.
									...
									# Translators: Announced when navigating past many non-contiguous hidden rows.
					...
					# ---------------- COLUMN JUMPS ----------------
					if not row_changed and col_changed:
						...
						if max_c - min_c > 1:
							...
							if hidden_count > 0:
								if not fragmented:
									if hidden_count == 1:
										# Translators: Announced when navigating past a single hidden column.
										...
										if first_hidden == min_c + 1 and last_hidden == max_c - 1 and (max_c - min_c - 1) >= 500:
											# Translators: Announced when navigating past multiple hidden columns.
									...
									# Translators: Announced when navigating past many non-contiguous hidden columns.
			...
			if radar_mode and radar_mode != "off":
				if (row_changed or col_changed) and _last_focused_row is not None and _last_focused_col is not None:
					...
					if is_empty:
						...
						try:
							# Moving Down
							if current_row > _last_focused_row and not col_changed:
								if current_row < max_row:
									rng = sheet.Range(sheet.Cells(current_row + 1, current_col), sheet.Cells(max_row, current_col))
									if not has_data(rng):
										# Translators: Announced when trying to jump to edge of data but there is none below.
										ui.message(_("No more data below"))
										
							# Moving Up
							elif current_row < _last_focused_row and not col_changed:
								if current_row > 1:
									rng = sheet.Range(sheet.Cells(1, current_col), sheet.Cells(current_row - 1, current_col))
									if not has_data(rng):
										# Translators: Announced when trying to jump to edge of data but there is none above.
										ui.message(_("No more data above"))
										
							# Moving Right
							elif current_col > _last_focused_col and not row_changed:
								if current_col < max_col:
									rng = sheet.Range(sheet.Cells(current_row, current_col + 1), sheet.Cells(current_row, max_col))
									if not has_data(rng):
										# Translators: Announced when trying to jump to edge of data but there is none to the right.
							...
							# Moving Left
								...
								if current_col > 1:
									...
									if not has_data(rng):
										# Translators: Announced when trying to jump to edge of data but there is none to the left.
		...
		try:
			...
			if active_win:
				...
				if _last_freeze_panes_state is not None and current_freeze != _last_freeze_panes_state:
					if current_freeze:
						# Translators: Announced when frozen panes are enabled.
						ui.message(_("Panes frozen"))
					else:
						# Translators: Announced when frozen panes are disabled.
			...
			try:
				...
				if (_last_structural_sheet is not None and 
					...
					if not found:
						...
						# Translators: Announced when a sheet is deleted.
						...
						for i in range(min_s + 1, max_s):
							try:
								...
								if sheet.Visible != -1:  # -1 is xlSheetVisible
									# Translators: Announced when a sheet is hidden.
				...
				if _last_visible_sheet_count is not None and current_visible != _last_visible_sheet_count:
					...
					if current_visible < _last_visible_sheet_count:
						...
						if not total_changed:
							# Translators: Announced when a sheet is unhidden.
	...
	def _execute_and_verify_visibility_change(self, gesture, element_type, isHiding, force_com=False):
		"""
		...
		try:
			...
			if excel:
				...
				if element_type == "row":
					...
					if force_com:
						sel.EntireRow.Hidden = isHiding
				elif element_type == "column":
					initial_state = sel.EntireColumn.Hidden
					if force_com:
						sel.EntireColumn.Hidden = isHiding
		except Exception:
			pass
		...
		core.callLater(200, self._verify_visibility_change_callback, element_type, isHiding, initial_state)
		
	def _verify_visibility_change_callback(self, element_type, isHiding, initial_state):
		"""
			...
			if excel:
				...
				if element_type == "row":
					...
					if start_row == end_row:
						# Translators: Describes a single row.
						address_str = _("Row {num}").format(num=start_row)
					else:
						# Translators: Describes a range of rows.
				...
				if is_hidden is not None and is_hidden != initial_state:
					# Translators: Describes the visibility state of rows or columns.
					state_str = _("hidden") if is_hidden else _("unhidden")
					# Translators: Announced when rows or columns visibility state changes.
